...
public class VerCompeticiones {
		...
		try {
			...
			while (rs.next()) {
				...
				if(dto.fecha_inicio_inscripcion != null && dto.fecha_fin_inscripcion !=null) {
					...
					LocalDate iniIns = dto.fecha_inicio_inscripcion;
	...
	public static boolean comprobarFecha(LocalDate iniFec, LocalDate actual, LocalDate finFec) {
		if ((actual.isAfter(iniFec) || actual.isEqual(iniFec)) && 
				(finFec.isAfter(actual)|| finFec.isEqual(actual)))
			return true;
		return false;
